...
6. Excluded
* This column can be marked **MOV**, **BIK**, **YES** or **NO**
* If **MOV**, the tool will skip checks for the file in **MOV** mode, but will still check it in **BIK** mode
* If **BIK**, the tool will skip checks for the file in **BIK** mode, but will still check it in **MOV** mode
* If **YES**, the tool will skip checks in both **MOV** and **BIK** mode
* If **NO**, the tool will check the file as normal in both modes
